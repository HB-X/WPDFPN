...
class BBoxHead(nn.Module):
    ...
    def forward(self, x_td, x_bu):
        if self.with_avg_pool:
            x_td = self.avg_pool(x_td)
            x_bu = self.avg_pool(x_bu)
        x_td = x_td.view(x_td.size(0), -1)
        x_bu = x_bu.view(x_bu.size(0), -1)
        cls_score = self.fc_cls(x_td) if self.with_cls else None
        bbox_pred = self.fc_reg(x_bu) if self.with_reg else None
