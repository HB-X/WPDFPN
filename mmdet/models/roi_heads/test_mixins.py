...
class BBoxTestMixin(object):
    ...
    if sys.version_info >= (3, 7):
        ...
        async def async_test_bboxes(self,
            ...
            roi_feats_td = self.bbox_roi_extractor(
                x_td[:len(self.bbox_roi_extractor.featmap_strides)], rois)
            roi_feats_bu = self.bbox_roi_extractor(
                x_bu[:len(self.bbox_roi_extractor.featmap_strides)], rois)
            if self.with_shared_head:
                roi_feats_td = self.shared_head(roi_feats_td)
                roi_feats_bu = self.shared_head(roi_feats_bu)
            sleep_interval = rcnn_test_cfg.get('async_sleep_interval', 0.017)

            async with completed(
                    __name__, 'bbox_head_forward',
                    sleep_interval=sleep_interval):
                cls_score, bbox_pred = self.bbox_head(roi_feats_td, roi_feats_bu)
    ...
    def aug_test_bboxes(self, feats_td, feats_bu, img_metas, proposal_list, rcnn_test_cfg):
        """Test det bboxes with test time augmentation."""
        aug_bboxes = []
        aug_scores = []
        for x_td, x_bu, img_meta in zip(feats_td, feats_bu, img_metas):
...
class MaskTestMixin(object):
        ...
        async def async_test_mask(self,
            ...
            if det_bboxes.shape[0] == 0:
                ...
                mask_feats_td = self.mask_roi_extractor(
                    x_td[:len(self.mask_roi_extractor.featmap_strides)],
                    mask_rois)
                mask_feats_bu = self.mask_roi_extractor(
                    x_bu[:len(self.mask_roi_extractor.featmap_strides)],
                    mask_rois)
    ...
    def aug_test_mask(self, feats_td, feats_bu, img_metas, det_bboxes, det_labels):
