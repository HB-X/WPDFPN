...
class RPNHead(RPNTestMixin, AnchorHead):
    ...
    def init_weights(self):
        ...
        normal_init(self.rpn_conv_cls, std=0.01)
        normal_init(self.rpn_conv_reg, std=0.01)
        normal_init(self.rpn_cls, std=0.01)
        normal_init(self.rpn_reg, std=0.01)

    def forward_single(self, x_td, x_bu):
        """Forward feature map of a single scale level."""
        x_td = self.rpn_conv_cls(x_td)
        x_bu = self.rpn_conv_reg(x_bu)
        x_td = F.relu(x_td, inplace=True)
        x_bu = F.relu(x_bu, inplace=True)
        rpn_cls_score = self.rpn_cls(x_td)
        rpn_bbox_pred = self.rpn_reg(x_bu)
